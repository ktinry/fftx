...
int main(int argc, char *argv[])
{
	...
	fftx_complex *in1  = test_create_complexdata_1d(n);
	fftx_complex *in2  = test_copy_complexdata_1d(n, in1);
	fftx_real    *out1 = fftx_internal_malloc(n * sizeof(fftx_real));
	fftx_real    *out2 = fftx_internal_malloc(n * sizeof(fftx_real));

	fftw_plan wplan = fftw_plan_dft_c2r_1d(n, in1, out1, wflags);
	fftw_execute(wplan);
	fftw_destroy_plan(wplan);

	fftx_plan xplan = fftx_plan_dft_c2r_1d(n, in2, out2, xflags | wflags);
	fftx_execute(xplan);
	fftx_destroy_plan(xplan);

	double maxdiff = test_compare_r1d(n, out1, out2);
