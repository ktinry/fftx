...
// Hello World example
//
// n point pruned real convolution with FFTX
//
// input pruning: assume only first n_s elements are non-zero 
// output pruning: only the last n_d elements are requested
//
// convolution kernel divides the values by 1/n, thus the convolution is the identity function
...
fftx_plan pruned_real_convolution_plan(fftx_real *in, fftx_real *out, fftx_complex *symbol, int n, int n_in, int n_out, int n_freq) {
	int rank = 1,			// 1D = rank 1
		batch_rank = 0,		// no batch, thus batch rank 0
		numsubplans = 5;	// need 5 FFTX subplans for pruned convolution
    // FFTX iodim definitions for 1D + pruning
	fftx_iodim padded_dims = { n, 1, 1 },		// zero-padding to n real elements at unit stride
		freq_dims = { n_freq, 1, 1 },				// n/2+1 complex elements in frequency domain for conjugate even
		batch_dims = { 1, 1, 1 };					// no batching
	fftx_iodimx in_dimx = { n_in, 0, 0, 0, 1, 1, 1 },	// input: n_in real elements at unit stride
		out_dimx = { n_out, n - n_out, 0, 0, 1, 1, 1 },		// output: n_out real elements at unit stridefreq_dimx = { n_freq, 0, 1, 1, 1 },	// for pointwise op
		freq_dimx = { n_freq, 0, 0, 0, 1, 1, 1 },			// n/2+1 complex elements in frequency domain for conjugate even
...
int main(void) {
	...
	int n = 8, 
		...
		n_out = 5,
		n_freq = n/2 + 1;
	fftx_iodim dims_in = { n_in, 1, 1 },
		dims_out = { n_out, 1, 1 },
		dims_freq = { n_freq, 1, 1 };
	double symval = 1.0 / ((double)n);
	double *planiodata[3];	// used for persistent calls

	fftx_init(MY_FFTX_MODE);

	// allocate input, symbol, and output
	planiodata[0] = in = (fftx_real*)fftx_create_data_real(1, &dims_in);
	planiodata[1] = out = (fftx_real*)fftx_create_data_real(1, &dims_out);
	symbol = (fftx_complex*)fftx_create_data_complex(1, &dims_freq);
	...
	printf("in = ");
	fftx_print_vector_real(in, n_in);

	printf("out = ");
	fftx_print_vector_real(out, n_out);
